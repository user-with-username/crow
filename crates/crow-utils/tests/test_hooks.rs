...
mod tests {
    ...
    fn test_run_hooks_large_output() {
        ...
        #[cfg(windows)]
        let hooks = vec!["cmd /c for /l %i in (1,1,100) do echo Line %i".to_string()];

        #[cfg(unix)]
        let hooks = vec!["sh -c 'for i in $(seq 1 100); do echo Line $i; done'".to_string()];
