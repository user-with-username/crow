...
impl Flags {
    ...
    pub fn define(&mut self, name: impl Into<String>, value: Option<impl Into<String>>) -> &mut Self {
        self.flags.push(Flag::Define(
            name.into(),
            value.map(|v| v.into())
        ));
        self
    ...
    pub fn add_raw(&mut self, raw_flag: impl AsRef<str>) -> &mut Self {
        ...
        if let Some(parsed) = Self::parse_raw(flag_str) {
            ...
            self.flags.push(Flag::MachineDependent(flag_str.to_string()));
        }
        self
    }
    ...
    fn parse_raw(flag: &str) -> Option<Flag> {
        ...
        if flag.starts_with("-D") || flag.starts_with("/D") {
            ...
            if let Some(pos) = def.find('=') {
                return Some(Flag::Define(def[..pos].to_string(), Some(def[pos+1..].to_string())));
            } else {
