...
impl MsvcToolchain {
    ...
    fn from_compiler_path(compiler_path: &str) -> Result<Self> {
        let compiler_path = PathBuf::from(compiler_path);
        ...
        let tools_root = compiler_path
            ...
            .and_then(|p| p.parent())
            .and_then(|p| p.parent())
            .ok_or_else(|| anyhow!("Could not derive tools root from compiler path"))?;
        ...
        Self::add_windows_sdk_paths(&mut includes, &mut libraries)?;
